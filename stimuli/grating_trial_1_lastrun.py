...
expName = 'grating_trial_1'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/burakgur/Documents/GitHub/PyStim/stimuli/grating_trial_1_lastrun.py',
...
# Initialize components for Routine "FFF"
FFFClock = core.Clock()
polygon = visual.Rect(
...
for thisTrial in Trials:
    ...
    # ------Prepare to start Routine "FFF"-------
    continueRoutine = True
    routineTimer.add(1.000000)
    # update component parameters for each repeat
    # keep track of which components have finished
    FFFComponents = [polygon]
    for thisComponent in FFFComponents:
    ...
    FFFClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = FFFClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=FFFClock)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if grating.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > grating.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
